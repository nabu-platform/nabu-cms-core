<template id="nabu-cms-core-reset">
	<div class="nabu nabu-cms nabu-cms-core-reset">
		<slot name="title"><h1 class="title">%{reset:Reset password}</h1></slot>
		...
		<n-form ref="form" v-if="!updated">
			...
			<footer slot="footer">
				<div class="actions">
					<button :disabled="!valid || working || password1 != password2" class="primary" @click="resetPassword">%{reset:Reset password}</button>
				</div>
				<n-messages :messages="messages"/>
			</footer>
		</n-form>
		<div v-if="updated" class="completed">
			<slot>%{reset:Your password has been reset, click to <a v-route:login>log in</a>}</slot>
		</div>
		<slot name="end"></slot>
	</div>
</template>