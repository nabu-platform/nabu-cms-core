Vue.service("user", {
	...
	methods: {
		...
		downloadUrl: function(operationId, parameters) {
			...
			if (operation) {
				...
				if (operation["x-temporary-id"] && operation["x-temporary-secret"]) {
					...
					this.$services.swagger.execute("nabu.cms.core.api.v2.web.security.otp", {
		...
		login: function(username, password, remember, type) {
			var self = this;
			var promise = this.$services.q.defer();
			this.$services.swagger.execute("nabu.cms.core.api.v2.web.security.login", { 
				body: {
		...
		logout: function() {
			var self = this;
			var promise = this.$services.q.defer();
			this.$services.swagger.execute("nabu.cms.core.api.v2.web.security.forget", {$$skipRemember: true}).then(function(result) {
				self.bearer = null;
				self.roles.splice(0);
				self.permissions.splice(0);
		...
		remember: function() {
			...
			if (this.remembering) {
				...
				this.$services.swagger.execute("nabu.cms.core.api.v2.web.security.remember", {$$skipRemember: true}).then(function(result) {
					if (result) {
						self.bearer = result.token;
						// set the bearer token for the swagger client if we have a jwt token
						self.$services.swagger.bearer = result.token;
